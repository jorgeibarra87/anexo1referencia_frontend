...
export default function EntidadesTable() {
    ...
    const openEdit = (entidad) => {
        setEditando(entidad);
        setFormData({ nombre: entidad.nombre, sigla: entidad.sigla || '', estado: entidad.estado });
        setShowForm(true);
    };
    ...
    const handleSave = async () => {
        ...
        if (editando) {
            await entidadService.update(editando.idEntidad, formData);
        } else {
            await entidadService.create(formData);
        }
        setShowForm(false);
        setEditando(null);
        loadEntidades();
    ...
    const paginated = entidades.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
    const totalPages = Math.ceil(entidades.length / itemsPerPage);

    return (
        <div className="w-full">
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-bold text-gray-800">Entidades / Sindicatos</h2>
                <button onClick={openCreate} className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 text-sm flex items-center gap-2">
                    <FontAwesomeIcon icon={faPlus} /> Nueva Entidad
                </button>
            </div>
            ...
            {showForm && (
                <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
                    <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-md">
                        <h3 className="text-lg font-bold mb-4">{editando ? 'Editar' : 'Nueva'} Entidad</h3>
            ...
            <div className="bg-white rounded-xl shadow-md overflow-hidden">
                <table className="w-full text-sm">
                    ...
                    <tbody>
                        {loading ? (
                            ...
                            <tr key={e.idEntidad} className="border-t border-gray-100 hover:bg-gray-50">
                                <td className="p-3">{e.idEntidad}</td>
                                <td className="p-3 font-medium">{e.nombre}</td>
                                <td className="p-3 text-gray-500">{e.sigla || '-'}</td>
                                <td className="p-3">
                                    <span className={`px-2 py-0.5 rounded text-xs ${e.estado ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                                        {e.estado ? 'Activo' : 'Inactivo'}
                                    </span>
                                </td>
                                <td className="p-3 text-center">
                                    <button onClick={() => openEdit(e)} className="text-blue-600 hover:text-blue-800 mx-1"><FontAwesomeIcon icon={faEdit} /></button>
                                    <button onClick={() => handleDelete(e.idEntidad)} className="text-red-600 hover:text-red-800 mx-1"><FontAwesomeIcon icon={faTrash} /></button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
                ...
                {totalPages > 1 && (
                    <div className="flex justify-between items-center p-3 border-t">
                        <span className="text-sm text-gray-500">{entidades.length} registros</span>
                        <div className="flex items-center gap-2">
